...
exports.sendAdminNotification = exports.updateDoctorAvailability = exports.cleanupStaleCalls = exports.sendCallNotification = exports.createHmsRoom = exports.getHmsToken = void 0;
...
// 100ms credentials
const hmsAccessKey = (0, params_1.defineSecret)("HMS_ACCESS_KEY");
const hmsAppSecret = (0, params_1.defineSecret)("HMS_APP_SECRET");
/**
 * Generate 100ms auth token for video calling
 * Creates a room and returns tokens for both caller and doctor
 */
exports.getHmsToken = (0, https_1.onCall)({ secrets: [hmsAccessKey, hmsAppSecret] }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    const { roomId, role, userId } = request.data;
    if (!roomId || !role || !userId) {
        throw new https_1.HttpsError("invalid-argument", "roomId, role, and userId are required");
    }
    try {
        ...
        throw new https_1.HttpsError("internal", "Failed to generate video call token");
    }
});
/**
 ...
 */
exports.createHmsRoom = (0, https_1.onCall)({ secrets: [hmsAccessKey, hmsAppSecret] }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    const { callId, callerId, doctorId } = request.data;
    if (!callId || !callerId || !doctorId) {
        throw new https_1.HttpsError("invalid-argument", "callId, callerId, and doctorId are required");
    }
    try {
        const accessKey = hmsAccessKey.value();
        const appSecret = hmsAppSecret.value();
        ...
        throw new https_1.HttpsError("internal", "Failed to create video call room");
    }
});
/**
 ...
 */
exports.sendCallNotification = (0, https_1.onCall)(async (request) => {
    // Verify user is authenticated
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated to send call notifications");
    }
    const { callId, doctorId, callerName, callerPhoto } = request.data;
    if (!callId || !doctorId) {
        throw new https_1.HttpsError("invalid-argument", "callId and doctorId are required");
    }
    try {
        // Get doctor's FCM token
        const doctorDoc = await db.collection("users").doc(doctorId).get();
        const doctorData = doctorDoc.data();
        if (!(doctorData === null || doctorData === void 0 ? void 0 : doctorData.fcmToken)) {
        ...
        throw new https_1.HttpsError("internal", "Failed to send call notification");
    }
});
/**
 ...
 */
exports.updateDoctorAvailability = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    try {
        ...
        if (!doctorProfile.exists) {
            throw new https_1.HttpsError("not-found", "Doctor profile not found");
        }
        ...
        throw new https_1.HttpsError("internal", "Failed to update availability");
    }
});
/**
...
exports.sendAdminNotification = (0, https_1.onCall)(async (request) => {
    ...
    console.log("Auth:", (_a = request.auth) === null || _a === void 0 ? void 0 : _a.uid);
    ...
    const { title, body, imageUrl, target, type, referenceId, referenceType, extraData, } = request.data;
    if (!title || !body) {
    ...
    try {
        ...
        if (!adminData || (adminData.role !== "admin" && adminData.role !== "creator")) {
            console.log("ERROR: User is not admin, role:", adminData === null || adminData === void 0 ? void 0 : adminData.role);
        ...
        for (let i = 0; i < tokens.length; i += batchSize) {
            ...
            if (response.failureCount > 0) {
                ...
                response.responses.forEach((resp, idx) => {
                    var _a, _b;
                    if (!resp.success) {
                        console.log(`Token ${idx} failed:`, (_a = resp.error) === null || _a === void 0 ? void 0 : _a.code, (_b = resp.error) === null || _b === void 0 ? void 0 : _b.message);
                    }
                });
            }
        }
